...
export default function TerminalUI() {
  ...
  const [output, setOutput] = useState<
    Array<{ text: string; isError?: boolean; isCommand?: boolean }>
  ...
  const handleArrowUp = () => {
    ...
    if (prevCommand !== undefined) {
      ...
      xtermRef.current.write("\r");
      xtermRef.current.write(
        " ".repeat(terminalConfig.promptSymbol.length + currentInput.length)
      );
      xtermRef.current.write("\r");
      xtermRef.current.write(terminalConfig.promptSymbol);
  ...
  const executeCommand = async (commandLine: string) => {
    ...
    try {
      ...
      if (loadingMessage) {
        xtermRef.current.write(
          "\r" + " ".repeat(loadingMessage.length - 1) + "\r"
        );
      }
      ...
      if (response.updateAccentColor) {
        if (command.toLowerCase() === "weather") {
          setAccentColor(
            getWeatherAccentColor(response.updateAccentColor as any)
          );
