...
function handleHelp(): CommandResponse {
  const helpOutput = [
    'Available Commands:',
    '',
    ...availableCommands.map(cmd => `  ${cmd.padEnd(10)} - ${commandDescriptions[cmd] || 'No description'}`),
    '',
...
async function handleWeather(): Promise<CommandResponse> {
  try {
    ...
    return {
      ...
      updateAccentColor: weatherData.condition ? weatherData.condition : undefined
    };
  } catch (error) {
    return {
      output: `Failed to get weather information: ${error instanceof Error ? error.message : 'Unknown error'}`,
      error: true
    };
  }
}
...
async function handleTime(): Promise<CommandResponse> {
  try {
    ...
    return {
      output: `Failed to get time information: ${error instanceof Error ? error.message : 'Unknown error'}`,
      error: true
    };
  }
}
...
async function handleTeachMe(args: string[]): Promise<CommandResponse> {
  if (!args.length) {
    return { 
      output: "Please provide a topic to learn about. For example: 'teachme blockchain'",
      error: true
    };
  }
  ...
  try {
    ...
    const response = await fetch(`/api/nomad/teachme?topic=${encodeURIComponent(topic)}`);
    ...
    return {
      output: `Failed to get explanation: ${error instanceof Error ? error.message : 'Unknown error'}`,
      error: true
    };
  }
}
