...
export type WeatherCondition = 'sunny' | 'clear' | 'cloudy' | 'rainy' | 'stormy' | 'snowy' | 'foggy' | 'default';
...
export function getWeatherAccentColor(condition: WeatherCondition): string {
  return nomadConfig.weather.conditionColors[condition] || nomadConfig.weather.conditionColors.default;
}
...
export function getTimeAccentColor(timeOfDay: TimeOfDay): string {
  return nomadConfig.weather.timeColors[timeOfDay] || nomadConfig.weather.timeColors.night;
}
...
export function formatWeatherOutput(weatherData: WeatherData): string {
  const { city, condition, temperature, temperatureUnit, localTime } = weatherData;
